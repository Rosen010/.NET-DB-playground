...
/// <summary>
/// Menu handler for complex reporting queries.
/// Demonstrates Dapper's capabilities with JOINs, aggregations, and complex SQL.
/// </summary>
public class ReportsMenu
{
    ...
    public ReportsMenu(IReportsRepository reportsRepository, IUserRepository userRepository)
    {
    ...
    public async Task ShowAsync()
    {
        while (true)
        {
            var choice = MenuHelper.ShowMenu("Reports & Analytics", new[]
            {
            ...
            {
                case 1:
                    await ShowMonthlySpendingAsync();
                    break;
                case 2:
                    await ShowAccountBalancesAsync();
                    break;
                case 3:
                    await ShowBudgetStatusAsync();
                    break;
                case 4:
                    await ShowIncomeExpensesSummaryAsync();
                    break;
                case 5:
                    await ShowTopSpendingCategoriesAsync();
                    break;
                case 6:
                    return;
                default:
    ...
    private async Task ShowMonthlySpendingAsync()
    {
        ...
        Console.WriteLine("This query demonstrates JOINs, GROUP BY, and aggregate functions.");
        Console.WriteLine();
        ...
        Console.WriteLine("\nAvailable users (enter 0 for all users):");
        ...
        var results = await _reportsRepository.GetMonthlySpendingByCategoryAsync(year, month, userId);
    ...
    private async Task ShowAccountBalancesAsync()
    {
        ...
        Console.WriteLine("This query demonstrates GROUP BY with multiple columns and aggregates.");
        Console.WriteLine();

        var results = await _reportsRepository.GetAccountBalanceSummaryAsync();
        ...
        foreach (var item in results)
        {
            ...
            {
                // Print subtotal for previous user
    ...
    private async Task ShowBudgetStatusAsync()
    {
        ...
        Console.WriteLine("This query demonstrates subqueries and budget vs actual comparison.");
        Console.WriteLine();

        var year = MenuHelper.PromptInt("Enter year (e.g., 2024)");
        var month = MenuHelper.PromptInt("Enter month (1-12)");

        var results = await _reportsRepository.GetBudgetStatusAsync(year, month);
    ...
    private async Task ShowIncomeExpensesSummaryAsync()
    {
        ...
        Console.WriteLine("This query demonstrates conditional aggregation with CASE expressions.");
        Console.WriteLine();
        ...
        var (totalIncome, totalExpenses, netAmount) = await _reportsRepository.GetIncomeExpenseSummaryAsync(startDate, endDate, userId);
        ...
        if (netAmount >= 0)
        {
            Console.WriteLine($"\n  You saved {(totalIncome > 0 ? (netAmount / totalIncome * 100) : 0):N1}% of your income!");
        }
        else
        {
            Console.WriteLine($"\n  Warning: You spent more than you earned this period.");
        }
    ...
    private async Task ShowTopSpendingCategoriesAsync()
    {
        ...
        Console.WriteLine("This query demonstrates LIMIT clause and user-specific filtering.");
        Console.WriteLine();
        ...
        Console.Write("How many top categories to show (default 5): ");
        var topCountInput = Console.ReadLine();
        var topCount = int.TryParse(topCountInput, out var tc) && tc > 0 ? tc : 5;

        var results = await _reportsRepository.GetTopSpendingCategoriesAsync(userId, topCount);
